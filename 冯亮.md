...
1. 下面的Python代码会输出什么。
   ...
   ```
   [('abcd',1),('abcd',2),('abcd',3),('abcd',4),('abcd',5)]
...
3. 有一个通过网络获取数据的Python函数（可能会因为网络或其他原因出现异常），写一个装饰器让这个函数在出现异常时可以重新执行，但尝试重新执行的次数不得超过指定的最大次数。
   ...
   ```Python
   def reload(func,n,*agrs,**kwargs):
       def ware(func,n,*args,**kwargs):
           try:
               result = func(*args,**kwargs)
           except:
               if n > 0:
                   n -= 1
                   ware(func,n,*args,**kwargs)
               return '多次获取失败'
           return result
        return ware(func,n,*agrs,**kwargs)
   ```
...
4. 下面的字典中保存了某些公司今日的股票代码及价格，用一句Python代码从中找出价格最高的股票对应的股票代码，用一句Python代码创建股票价格大于100的股票组成的新字典。
   ...
   ```Python
   max(prices,lambda x :prices[x] for x in prices )
   {key : value for (key,value) in prices if value >100 }
   ```
...
5. 写一个函数，传入的参数是一个列表，如果列表中的三个元素`a`、`b`、`c`相加之和为`0`，就将这个三个元素组成一个三元组，最后该函数返回一个包含了所有这样的三元组的列表。例如：
   ...
   ```Python
	def zero_sum(nums:list):
		result=[]
		for a in range(len(nums)-2):
			for b in range(len(nums)-a-1):
				for c in range(len(nums)-b-1):
					if nums[a]+nums[a+b+1]+nums[a+b+c+2]:
						continue
					result.append((nums[a],nums[a+b+1],nums[a+b+c+2]))
		return result
...
6. 写一个函数，传入的参数是一个列表（列表中的元素可能也是一个列表），返回该列表最大的嵌套深度，例如：
   ...
   ```Python
def deep(the_list, n=1, n_list=[1]):
	m = n
	for item in the_list:
		n = m
		if isinstance(item, list):
			n += 1
			n, n_list = deep(item, n, n_list)
		if n not in n_list:
			n_list.append(n)
	return max(n_list), n_list
   ```
...
10. 在MySQL数据库中有名为`tb_result`的表如下所示，请写出能查询出如下所示结果的SQL。
    ...
    ```SQL
    select rq ,'胜' ,'负' from rq, count(shengfu) as '胜' from tb_result  where shengfu = '胜' group by rq tb1 inner join rq, count(shengfu)as '负' from tb_result  where shengfu = '负' group by rq t2 on tb1.rq=tb2.rq;  
    ```
